...
paymentForm.include({
    ...
    _wompicolPayEvent: function (ev) {
        ...
        if (checked_radio.length === 1) {
            ...
            var providerId = this.getProviderId(checked_radio);
            var providerForm = false;
            if (this.isNewPaymentRadio(checked_radio)) {
                providerForm = this.$('#o_payment_add_token_acq_' + providerId);
            } else {
                providerForm = this.$('#o_payment_form_acq_' + providerId);
            }
            var inputs_form = $('input', providerForm);
            ...
            // if the user is going to pay with a form payment, then
                ...
                if ($tx_url.length === 1) {
                    ...
                    var form_save_token = providerForm.find('input[name="o_payment_form_save_token"]').prop('checked');
    ...
    // Helper method for Odoo 19 compatibility
    getProviderId: function(radio) {
        // In Odoo 19, use provider_id instead of acquirer_id
        return this.getAcquirerIdFromRadio ? this.getAcquirerIdFromRadio(radio) : 
               radio.dataset.providerId || radio.dataset.acquirerId;
    },
})
